# tests/integration/diagnosis/conftest.py
"""Testcontainers-backed Postgres + Kafka for diagnosis integration tests.

No session-level _migrate autouse here — the round-trip test owns the
migration lifecycle itself (upgrade head → downgrade -1 → upgrade head).
The end-to-end test calls ``command.upgrade`` inline so it owns the full
...
def _migrate(pg_container: PostgresContainer) -> None:
    ...
    fails if called from inside a running event loop. Mirrors the pattern in
    ``tests/integration/enrichment/conftest.py``.

    The round-trip test runs its own upgrade/downgrade/upgrade cycle; that's
    fine because it uses a separate ``pg_dsn`` call and pytest runs tests
    sequentially within the session by default.
    """
...
def _reset_state(pg_dsn: str) -> Iterator[None]:
    ...
    The pg_container is session-scoped; per-test we just clear data.
    CASCADE handles FK refs from diagnoses → incidents.
