"""Testcontainers fixtures for the load suite.

Boots the real app (Postgres + Redis + Kafka) with the diagnosis and memory
consumers DISABLED, so load traffic exercises the synchronous
webhook → persist → outbox path only and never reaches the Anthropic API.
Mirrors tests/integration/ingestion/conftest.py.
"""
...
GENERIC_SECRET = "loadtest-secret"
