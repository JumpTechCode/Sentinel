...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("Connection refused (request_id=abcdef1234567890)", "connection refused"),
...
@pytest.mark.parametrize(
    "raw,expected",
    [
        # Trailing punctuation on individual tokens is stripped.
        ("a- b__ c.", "a b c"),
        # Empty tokens (pure punctuation after substitutions) are dropped.
        ("foo - bar", "foo bar"),
        # Lone ')' left after an "=" — the old orphan-close pass still triggers.
        ("retry x= ) failed", "retry x= failed"),
        # Interior punctuation in a legitimate token is preserved.
        ("foo.bar baz", "foo.bar baz"),
    ],
)
def test_normalize_title_cleanup_pass(raw: str, expected: str) -> None:
    assert normalize_title(raw) == expected
